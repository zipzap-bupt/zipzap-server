...
(function() {
    ...
    (function() { window[b.guid] = {};
        ...
        b.lang.Event = function(g, h) { this.type = g;
        ...
        b.lang.Class.prototype.addEventListener = function(j, i, h) {
            ...
            if (typeof h == "string" && h) {
                if (/[^\w\-]/.test(h)) {
                    throw ("nonstandard key:" + h) } else { i.hashCode = h;
                    k = h } }
        ...
        b.lang.Class.prototype.removeEventListener = function(i, h) {
            if (b.lang.isFunction(h)) { h = h.hashCode } else {
        ...
        b.g = b.dom.g = function(g) {
            if ("string" == typeof g || g instanceof String) {
                return document.getElementById(g) } else {
        ...
        b.dom._NAME_ATTRS = (function() {
            var g = { cellpadding: "cellPadding", cellspacing: "cellSpacing", colspan: "colSpan", rowspan: "rowSpan", valign: "vAlign", usemap: "useMap", frameborder: "frameBorder" };
            if (b.browser.ie < 8) { g["for"] = "htmlFor";
                g["class"] = "className" } else { g.htmlFor = "for";
                g.className = "class" }
            return g })();
        b.getAttr = b.dom.getAttr = function(h, g) { h = b.dom.g(h);
            if ("style" == g) {
        ...
        b.on = b.event.on = function(h, k, m) { k = k.replace(/^on/i, "");
            h = b.g(h);
            ...
            if (j && j[k]) { n = j[k](h, k, l);
                i = n.type;
                l = n.listener }
            if (h.addEventListener) { h.addEventListener(i, l, false) } else {
                if (h.attachEvent) { h.attachEvent("on" + i, l) } }
            g[g.length] = [h, k, m, l, i];
            return h } })();
    var f = { request: function(j, g) {
            if (g) {
                var i = (Math.random() * 100000).toFixed(0);
                BMap._rd["_cbk" + i] = function(k) { g && g(k);
                    delete BMap._rd["_cbk" + i] };
                j += "&callback=BMap._rd._cbk" + i }
            var h = document.createElement("script");
            h.setAttribute("src", j);
            document.body.appendChild(h);
            if (h.addEventListener) { h.addEventListener("load", function(l) {
                    var k = l.target;
                    k.parentNode.removeChild(k) }, false) } else {
                if (h.attachEvent) { h.attachEvent("onreadystatechange", function(l) {
                        var k = window.event.srcElement;
                        if (k && (k.readyState == "loaded" || k.readyState == "complete")) { k.parentNode.removeChild(k) } }) } }
            setTimeout(function() { document.getElementsByTagName("head")[0].appendChild(h);
                h = null }, 1) } };
    var a = { serviceUrl: "http://api.map.baidu.com" };
    ...
    function c(n) {
        ...
        for (var k = 0, h = n.length; k < h; k++) {
            ...
            l.push(g) }
        return l }
    var d = BMapLib.CityList = function(g) { g = g || {};
        this._opts = b.extend(b.extend(this._opts || {}, { container: null, map: null }), g);
        this._data = null;
        this._init() };
    b.lang.inherits(d, b.lang.Class, "CityList");
    d.prototype._init = function() {
        if (this._opts.container) { this._initContainer() } };
    d.prototype.getBusiness = function(i, g) {
        var h = a.serviceUrl + "/shangquan/reverse/?wd=" + i;
        f.request(h, function(m) {
            var k = null;
            if (m && m.error && m.error["errno"] == "0") { k = m.content }
            for (var l = 0, j = k.length; l < j; l++) { k[l]["coordinate"] = c(k[l]["coordinate"]) }
            if (g) { g(k) } }) };
    d.prototype.getSubAreaList = function(h, g) {
        ...
        f.request(i, function(m) {
            var k = null;
            if (m && m.result && m.result["error"] == "0") { k = m.content }
            k.geo = e(k.geo);
            for (var l = 0, j = k.sub.length; l < j; l++) { k.sub[l]["geo"] = e(k.sub[l]["geo"]) }
            if (g) { g(k) } }) };
    d.prototype._initContainer = function() {
        var q = this;

        function h(x, z, y, u) { h.id = z;
            h.level = y;
            var w = new Date().getTime();
            var v = l + "&areacode=" + x;
            if (u) { v += "&business_flag=1" } else { v += "&business_flag=0" }
            f.request(v, k) }
        ...
        function k(z) {
            ...
            if (z.content) {
                ...
                if (z.content.area_code == 1) {
                    ...
                    map.centerAndZoom(E, 12) } else {
                    var E = new BMap.Point(v.lng, v.lat);
                    map.centerAndZoom(E, h.level) }
                if (z.content.sub) { D = C.sub;
                    D.splice(0, 0, { area_name: "请选择", area_code: "" });
                    var B = document.createDocumentFragment();
                    var t;
                    var x = { 131: 1, 289: 2, 332: 3, 132: 4 };
                    for (var w = 0; w < D.length; w++) { D[w].sort = D[w].area_code ? x[D[w].area_code] || 5 : 0 }
                    D.sort(function(G, F) {
                        return G.sort - F.sort });
                    for (var w = 0; w < D.length; w++) { t = document.createElement("option");
                        t.innerHTML = D[w].area_name;
                        t.area_type = D[w].area_type;
                        t.geo = D[w].geo;
                        if (D[w].business_geo) { t.business_geo = D[w].business_geo }
                        t.value = D[w].area_code;
                        t.title = D[w].area_name;
                        B.appendChild(t) }
                    D.shift();
                    if (h.id) { h.id.innerHTML = "";
                        h.id.appendChild(B) } } } }
        ...
        function o(t) { q.dispatchEvent("cityclick", { area_code: t.value, area_type: t.area_type, geo: e(t.geo), area_name: t.title }) }
